...
alter table ProductMember add column c_image varchar(100);
...
select sum(totalPrice) from Ordertable where managerId="ehgus191";
select * from Ordertable;
insert into Ordertable value("1","prodcut01","강아지 배변패드","1","15000","2024-02-29","15000","ehgus191","ehgus191","처리중");
insert into Ordertable value("2","prodcut01","강아지 배변패드","1","15000","2024-02-29","15000","ehgus191","ehgus191","처리중");
...
alter table HospitalMember add column image varchar(100);
-- productReview
...
create table EApllication(
    ...
    BoardId varchar(50) not null,
    alarm varchar(10) not null -- before,after 구분 
)default charset=utf8;
use 123team_db;
select * from EApllication where PersonId='ehgus191';
...
insert into ENBoard values("id1","내용1","1","2000-01-01","1","1");
insert into ENBoard values("id2","내용2","2","2","2000-02-02","2","1");
insert into ENBoard values("id3","내용3","3","3","2000-03-03","3");
insert into ENBoard values("id4","내용4","4","4","2000-04-04","4");
...
create table if not exists Product(
    ...
    PersonId varchar(10),
    AnimalCategory varchar(10) not null, -- 강아지,고양이,토끼,기니피그,파춯류,기타
        reviewScore float,
    reviewCount int
)default charset=utf8;

SELECT * FROM hospital order by reviewScore desc limit 3;
...
select * from hospital where reviewScore=4;
use 123team_db;
...
select * from HApllication ;
select * from HApllication where personId='ehgus190' and alarm='before';
...
select avg(reviewScore) from HospitalReview where hid="병원아이디1";
select * from hospital where reviewScore>=3.0 and reviewScore<4.0;
select * from HospitalReview R inner join hospital H on R.hid = H.hid where H.reviewScore=5;